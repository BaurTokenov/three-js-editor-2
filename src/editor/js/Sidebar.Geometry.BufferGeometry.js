import { UIRow, UIText, UISpan, UIBreak } from './libs/ui';
  ...
  function update(object) {
    ...
    if (geometry && geometry.isBufferGeometry) {
      ...
      if (index !== null) {
        ...
        container2.add(new UIText(index.count.toLocaleString()).setFontSize('12px'));
      ...
      for (const name in attributes) {
        ...
        container2.add(
          new UIText(attribute.count.toLocaleString() + ' (' + attribute.itemSize + ')').setFontSize('12px')
        );
        container2.add(new UIBreak());
      }
    } else {
