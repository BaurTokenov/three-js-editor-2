...
(function (mod) {
  ...
  CodeMirror.defineExtension('showHint', function (options) {
    ...
    if (this.somethingSelected()) {
      ...
      for (var i = 0; i < selections.length; i += 1) if (selections[i].head.line != selections[i].anchor.line) return;
    }
  ...
  function applicableHelpers(cm, helpers) {
    ...
    var result = [];
    for (var i = 0; i < helpers.length; i += 1) if (helpers[i].supportsSelection) result.push(helpers[i]);
    return result;
  }
