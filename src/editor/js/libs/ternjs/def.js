...
(function (mod) {
  ...
  function computedFunc(args, retType) {
    return function (self, cArgs) {
      var realArgs = [];
      for (var i = 0; i < args.length; i += 1) realArgs.push(unwrapType(args[i], self, cArgs));
  ...
  function computedUnion(types) {
    return function (self, args) {
      ...
      for (var i = 0; i < types.length; i += 1) unwrapType(types[i], self, args).propagate(union);
      return union;
    };
  }
  ...
  TypeParser.prototype = {
    ...
    parseType: function (comp, name, top) {
      ...
      var union = new infer.AVal();
      for (var i = 0; i < types.length; i += 1) types[i].propagate(union);
      return union;
    },
  ...
  function runPasses(type, arg) {
    ...
    if (pass) for (var i = 0; i < pass.length; i += 1) pass[i](arg);
  }
