...
(function (root, mod) {
  ...
  if (typeof define == 'function' && define.amd)
    // AMD
    return define(
      ['exports', 'acorn/dist/acorn', 'acorn/dist/acorn_loose', 'acorn/dist/walk', './def', './signal'],
      mod
    );
    ...
    toString: function (maxDepth) {
      ...
      for (var prop in this.props)
        if (prop != '<i>') {
          ...
          if (maxDepth) props.push(prop + ': ' + toString(this.props[prop], maxDepth - 1, this));
      ...
      if (node.left.type == 'MemberExpression') {
        ...
        if (pName == '<i>') {
          ...
          if (over) {
            ...
            var fromRight =
              node.right.type == 'MemberExpression' && node.right.computed && node.right.property.name == v;
  ...
  exports.purge = function (origins, start, end) {
    ...
    for (var prop in cx.props) {
      ...
      for (var i = 0; i < list.length; ++i) {
        var obj = list[i],
          av = obj.props[prop];
        if (!av || test(av, av.originNode)) list.splice((i -= 1), 1);
      }
  ...
  AVal.prototype.purge = function (test) {
    ...
    for (var i = 0; i < this.types.length; ++i) {
      var type = this.types[i];
      if (test(type, type.originNode)) this.types.splice((i -= 1), 1);
