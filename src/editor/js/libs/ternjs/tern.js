...
(function (root, mod) {
  ...
  function resolveFile(srv, localFiles, name) {
    ...
    infer.withContext(srv.cx, function () {
      ...
      if ((m = text.match(/(?:"([^"]*)"|([\w$]+))\s*:\s*function\b/))) {
        ...
        if (objNode && objNode.node.objType) inObject = { type: objNode.node.objType, prop: m[2] || m[1] };
      }
  ...
  function isPosition(val) {
    return (
      typeof val == 'number' || (typeof val == 'object' && typeof val.line == 'number' && typeof val.ch == 'number')
    );
  }
