...
class ConvexObjectBreaker {
  constructor(minSizeForBreak = 1.4, smallDelta = 0.0001) {
    ...
    this.segments = [];
    const n = 30 * 30;
    for (let i = 0; i < n; i += 1) this.segments[i] = false;
  }
  ...
  cutByPlane(object, plane, output) {
    ...
    for (let i = 0; i < numPointPairs; i += 1) this.segments[i] = false;
    ...
    if (numPoints1 > 0) {
      for (let i = 0; i < numPoints1; i += 1) this.tempCM1.add(points1[i]);
    ...
    if (numPoints2 > 0) {
      for (let i = 0; i < numPoints2; i += 1) this.tempCM2.add(points2[i]);
