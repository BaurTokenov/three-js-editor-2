...
class WebGPUUniformsGroup extends WebGPUUniformBuffer_1.default {
    constructor(name) {
        super(name);
    ...
    getByteLength() {
        ...
        for (let i = 0, l = this.uniforms.length; i < l; i += 1) {
            const uniform = this.uniforms[i];
            // offset within a single chunk in bytes
            const chunkOffset = offset % constants_1.GPUChunkSize;
            const remainingSizeInChunk = constants_1.GPUChunkSize - chunkOffset;
            // conformance tests
            if (chunkOffset !== 0 && remainingSizeInChunk - uniform.boundary < 0) {
                // check for chunk overflow
                offset += constants_1.GPUChunkSize - chunkOffset;
            }
