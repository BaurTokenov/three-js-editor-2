...
const MD2Loader_1 = require("../loaders/MD2Loader");
const MorphBlendMesh_1 = require("../misc/MorphBlendMesh");
class MD2CharacterComplex {
    constructor() {
        ...
        // rig
        this.root = new three_1.Object3D();
        this.meshBody = null;
    ...
    shareParts(original) {
        ...
        // WEAPONS
        for (let i = 0; i < original.weapons.length; i += 1) {
    ...
    loadParts(config) {
        ...
        function loadTextures(baseUrl, textureUrls) {
            ...
            const textures = [];
            for (let i = 0; i < textureUrls.length; i += 1) {
                textures[i] = textureLoader.load(baseUrl + textureUrls[i], checkLoadingComplete);
                textures[i].mapping = three_1.UVMapping;
                textures[i].name = textureUrls[i];
                textures[i].encoding = three_1.sRGBEncoding;
            }
            return textures;
        }
        ...
        for (let i = 0; i < config.weapons.length; i += 1)
            weaponsTextures[i] = config.weapons[i][1];
        // SKINS
        this.skinsBody = loadTextures(config.baseUrl + 'skins/', config.skins);
        this.skinsWeapon = loadTextures(config.baseUrl + 'skins/', weaponsTextures);
        // BODY
        const loader = new MD2Loader_1.MD2Loader();
    ...
    updateBehaviors() {
        ...
        if (Math.abs(this.speed) < 0.2 * this.maxSpeed &&
    ...
    updateMovementModel(delta) {
        ...
        if (controls.moveForward)
            this.speed = three_1.MathUtils.clamp(this.speed + delta * this.frontAcceleration, this.maxReverseSpeed, this.maxSpeed);
        if (controls.moveBackward)
            this.speed = three_1.MathUtils.clamp(this.speed - delta * this.backAcceleration, this.maxReverseSpeed, this.maxSpeed);
        // orientation based on controls
        // (don't just stand while turning)
        const dir = 1;
        if (controls.moveLeft) {
            this.bodyOrientation += delta * this.angularSpeed;
            this.speed = three_1.MathUtils.clamp(this.speed + dir * delta * this.frontAcceleration, this.maxReverseSpeed, this.maxSpeed);
        }
        ...
        if (!(controls.moveForward || controls.moveBackward)) {
            if (this.speed > 0) {
                ...
                this.speed = three_1.MathUtils.clamp(this.speed - k * delta * this.frontDecceleration, 0, this.maxSpeed);
            }
            else {
                const k = exponentialEaseOut(this.speed / this.maxReverseSpeed);
                this.speed = three_1.MathUtils.clamp(this.speed + k * delta * this.backAcceleration, this.maxReverseSpeed, 0);
            }
        }
        // displacement
    ...
    _createPart(geometry, skinMap) {
        const materialWireframe = new three_1.MeshLambertMaterial({ color: 0xffaa00, wireframe: true });
        const materialTexture = new three_1.MeshLambertMaterial({ color: 0xffffff, wireframe: false, map: skinMap });
        //
        const mesh = new MorphBlendMesh_1.MorphBlendMesh(geometry, materialTexture);
