...
const _m1 = new three_1.Matrix4();
const _obj = new three_1.Object3D();
const _offset = new three_1.Vector3();
class Geometry extends three_1.EventDispatcher {
    constructor() {
        super();
    ...
    applyMatrix4(matrix) {
        const normalMatrix = new three_1.Matrix3().getNormalMatrix(matrix);
    ...
    computeFaceNormals() {
        const cb = new three_1.Vector3(), ab = new three_1.Vector3();
    ...
    computeMorphNormals() {
        ...
        for (let i = 0, il = this.morphTargets.length; i < il; i++) {
            ...
            if (!this.morphNormals[i]) {
                ...
                for (let f = 0, fl = this.faces.length; f < fl; f++) {
                    const faceNormal = new three_1.Vector3();
                    const vertexNormals = { a: new three_1.Vector3(), b: new three_1.Vector3(), c: new three_1.Vector3() };
    ...
    toBufferGeometry() {
        ...
        for (const name in geometry.morphTargets) {
            ...
            for (let i = 0, l = morphTargets.length; i < l; i++) {
                ...
                const attribute = new three_1.Float32BufferAttribute(morphTarget.data.length * 3, 3);
        ...
        if (geometry.skinIndices.length > 0) {
            const skinIndices = new three_1.Float32BufferAttribute(geometry.skinIndices.length * 4, 4);
            buffergeometry.setAttribute('skinIndex', skinIndices.copyVector4sArray(geometry.skinIndices));
        }
        if (geometry.skinWeights.length > 0) {
            const skinWeights = new three_1.Float32BufferAttribute(geometry.skinWeights.length * 4, 4);
    ...
    static createBufferGeometryFromObject(object) {
        let buffergeometry = new three_1.BufferGeometry();
        const geometry = object.geometry;
        if (object.isPoints || object.isLine) {
            const positions = new three_1.Float32BufferAttribute(geometry.vertices.length * 3, 3);
            const colors = new three_1.Float32BufferAttribute(geometry.colors.length * 3, 3);
            buffergeometry.setAttribute('position', positions.copyVector3sArray(geometry.vertices));
            buffergeometry.setAttribute('color', colors.copyColorsArray(geometry.colors));
            if (geometry.lineDistances && geometry.lineDistances.length === geometry.vertices.length) {
                const lineDistances = new three_1.Float32BufferAttribute(geometry.lineDistances.length, 1);
...
class Face3 {
    constructor(a, b, c, normal, color, materialIndex = 0) {
        ...
        this.b = b;
        this.c = c;
        this.normal = (normal && normal.isVector3) ? normal : new three_1.Vector3();
        this.vertexNormals = Array.isArray(normal) ? normal : [];
        this.color = (color && color.isColor) ? color : new three_1.Color();
