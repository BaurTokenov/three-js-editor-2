...
exports.TiltLoader = void 0;
const three_1 = require("three");
const fflate = __importStar(require("../libs/fflate.module"));
class TiltLoader extends three_1.Loader {
    ...
    parse(buffer) {
        ...
        for (const brush_index in brushes) {
            ...
            group.add(new three_1.Mesh(geometry, material));
        }
        return group;
    }
}
exports.TiltLoader = TiltLoader;
class StrokeGeometry extends three_1.BufferGeometry {
    constructor(strokes) {
        super();
        const vertices = [];
        const colors = [];
        ...
        const prevPosition = new three_1.Vector3();
        ...
        this.setAttribute('color', new three_1.BufferAttribute(new Float32Array(colors), 4));
        this.setAttribute('uv', new three_1.BufferAttribute(new Float32Array(uvs), 2));
    }
}
...
function getShaders() {
    if (shaders === null) {
        const loader = new three_1.TextureLoader().setPath('./textures/tiltbrush/');
        shaders = {
            Light: {
                uniforms: {
...
function getMaterial(GUID) {
    ...
    switch (name) {
        case 'Light':
            return new three_1.RawShaderMaterial(getShaders().Light);
        default:
            return new three_1.MeshBasicMaterial({ vertexColors: true, side: three_1.DoubleSide });
    }
}
