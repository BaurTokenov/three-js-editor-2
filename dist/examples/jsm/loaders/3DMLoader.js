...
class Rhino3dmLoader extends three_1.Loader {
    constructor(manager) {
        super(manager);
        this.libraryPath = '';
    ...
    _createMaterial(material) {
        ...
        const diffusecolor = new three_1.Color(_diffuseColor.r / 255.0, _diffuseColor.g / 255.0, _diffuseColor.b / 255.0);
        ...
        const mat = new three_1.MeshStandardMaterial({
            color: diffusecolor,
            name: material.name,
            side: 2,
    ...
    _createGeometry(data) {
        // console.log(data);
        const object = new three_1.Object3D();
        ...
        for (let i = 0; i < instanceDefinitions.length; i++) {
            ...
            for (let j = 0; j < instanceReferences.length; j++) {
                ...
                if (iRef.geometry.parentIdefId === iDef.attributes.id) {
                    const iRefObject = new three_1.Object3D();
    ...
    _createObject(obj, mat) {
        const loader = new three_1.BufferGeometryLoader();
        const attributes = obj.attributes;
        ...
        switch (obj.objectType) {
            ...
            case 'PointSet':
                ...
                if (geometry.attributes.hasOwnProperty('color')) {
                    material = new three_1.PointsMaterial({ vertexColors: true, sizeAttenuation: false, size: 2 });
                }
                else {
                    _color = attributes.drawColor;
                    color = new three_1.Color(_color.r / 255.0, _color.g / 255.0, _color.b / 255.0);
                    material = new three_1.PointsMaterial({ color: color, sizeAttenuation: false, size: 2 });
                }
                material = this._compareMaterials(material);
                const points = new three_1.Points(geometry, material);
            ...
            case 'Brep':
                ...
                const mesh = new three_1.Mesh(geometry, mat);
            ...
            case 'Curve':
                ...
                _color = attributes.drawColor;
                color = new three_1.Color(_color.r / 255.0, _color.g / 255.0, _color.b / 255.0);
                material = new three_1.LineBasicMaterial({ color: color });
                material = this._compareMaterials(material);
                const lines = new three_1.Line(geometry, material);
            ...
            case 'Light':
                ...
                switch (geometry.lightStyle.name) {
                    ...
                    case 'LightStyle_WorldRectangular':
                        light = new three_1.RectAreaLight();
                        const width = Math.abs(geometry.width[2]);
                        const height = Math.abs(geometry.length[0]);
                        light.position.set(geometry.location[0] - (height / 2), geometry.location[1], geometry.location[2] - (width / 2));
                        light.height = height;
                        light.width = width;
                        light.lookAt(new three_1.Vector3(geometry.direction[0], geometry.direction[1], geometry.direction[2]));
                        break;
