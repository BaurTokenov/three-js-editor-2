...
function decodePrwm(buffer) {
    ...
    let flags = array[1];
    const indexedGeometry = !!(flags >> 7 & 0x01), indicesType = flags >> 6 & 0x01, bigEndian = (flags >> 5 & 0x01) === 1, attributesNumber = flags & 0x1F;
...
class PRWMLoader extends three_1.Loader {
    constructor(manager) {
        super(manager);
    }
    ...
    parse(arrayBuffer) {
        const data = decodePrwm(arrayBuffer), attributesKey = Object.keys(data.attributes), bufferGeometry = new three_1.BufferGeometry();
        for (let i = 0; i < attributesKey.length; i++) {
            const attribute = data.attributes[attributesKey[i]];
            bufferGeometry.setAttribute(attributesKey[i], new three_1.BufferAttribute(attribute.values, attribute.cardinality, attribute.normalized));
        }
        if (data.indices !== null) {
            bufferGeometry.setIndex(new three_1.BufferAttribute(data.indices, 1));
        }
