 ...
 */
class PVRLoader extends three_1.CompressedTextureLoader {
    constructor(manager) {
        super(manager);
    }
...
function _parseV3(pvrDatas) {
    ...
    let bpp, format;
    const metaLen = header[12], pixelFormat = header[2], height = header[6], width = header[7], 
    // numSurfs = header[ 9 ],
    numFaces = header[10], numMipmaps = header[11];
    switch (pixelFormat) {
        case 0: // PVRTC 2bpp RGB
            bpp = 2;
            format = three_1.RGB_PVRTC_2BPPV1_Format;
            break;
        case 1: // PVRTC 2bpp RGBA
            bpp = 2;
            format = three_1.RGBA_PVRTC_2BPPV1_Format;
            break;
        case 2: // PVRTC 4bpp RGB
            bpp = 4;
            format = three_1.RGB_PVRTC_4BPPV1_Format;
            break;
        case 3: // PVRTC 4bpp RGBA
            bpp = 4;
            format = three_1.RGBA_PVRTC_4BPPV1_Format;
            break;
        default:
...
function _parseV2(pvrDatas) {
    const header = pvrDatas.header;
    const headerLength = header[0], height = header[1], width = header[2], numMipmaps = header[3], flags = header[4], 
    // dataLength = header[ 5 ],
    // bpp =  header[ 6 ],
    // bitmaskRed = header[ 7 ],
    ...
    if (formatFlags === PVRTC_4) {
        format = _hasAlpha ? three_1.RGBA_PVRTC_4BPPV1_Format : three_1.RGB_PVRTC_4BPPV1_Format;
        bpp = 4;
    }
    else if (formatFlags === PVRTC_2) {
        format = _hasAlpha ? three_1.RGBA_PVRTC_2BPPV1_Format : three_1.RGB_PVRTC_2BPPV1_Format;
        bpp = 2;
    }
    else {
