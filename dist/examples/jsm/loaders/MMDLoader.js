...
const MMDToonShader_1 = require("../shaders/MMDToonShader");
const TGALoader_1 = require("../loaders/TGALoader");
const mmdparser_module_1 = require("../libs/mmdparser.module");
/**
 * Dependencies
...
class MMDLoader extends three_1.Loader {
    ...
    loadVMD(url, onLoad, onProgress, onError) {
        ...
        this.loader
            ...
            .setRequestHeader(this.requestHeader)
            .setWithCredentials(this.withCredentials);
        for (let i = 0, il = urls.length; i < il; i += 1) {
    ...
    _getParser() {
        if (this.parser === null) {
            if (typeof mmdparser_module_1.MMDParser === 'undefined') {
                throw new Error('THREE.MMDLoader: Import MMDParser https://github.com/takahirox/mmd-parser');
            }
            this.parser = new mmdparser_module_1.MMDParser.Parser(); // eslint-disable-line no-undef
        }
        return this.parser;
    }
}
...
const NON_ALPHA_CHANNEL_FORMATS = [
    three_1.RGB_S3TC_DXT1_Format,
    three_1.RGB_PVRTC_4BPPV1_Format,
    three_1.RGB_PVRTC_2BPPV1_Format,
    three_1.RGB_ETC1_Format,
    three_1.RGB_ETC2_Format,
];
...
class GeometryBuilder {
    ...
    build(data) {
        ...
        for (let i = 0; i < data.metadata.vertexCount; i += 1) {
        ...
        // indices
        for (let i = 0; i < data.metadata.faceCount; i += 1) {
        ...
        // groups
        for (let i = 0; i < data.metadata.materialCount; i += 1) {
        ...
        // bones
        for (let i = 0; i < data.metadata.rigidBodyCount; i += 1) {
        ...
        for (let i = 0; i < data.metadata.boneCount; i += 1) {
        ...
        if (data.metadata.format === 'pmd') {
            for (let i = 0; i < data.metadata.ikCount; i += 1) {
                const ik = data.iks[i];
                const param = {
                    target: ik.target,
                ...
                for (let j = 0, jl = ik.links.length; j < jl; j++) {
                    ...
                    if (data.bones[link.index].name.indexOf('ひざ') >= 0) {
                        link.limitation = new three_1.Vector3(1.0, 0.0, 0.0);
                    }
            ...
            for (let i = 0; i < data.metadata.boneCount; i += 1) {
                ...
                for (let j = 0, jl = ik.links.length; j < jl; j++) {
                    ...
                    if (ik.links[j].angleLimitation === 1) {
                        ...
                        rotationMin[0] = tmp1;
                        rotationMin[1] = tmp2;
                        link.rotationMin = new three_1.Vector3().fromArray(rotationMin);
                        link.rotationMax = new three_1.Vector3().fromArray(rotationMax);
                    }
                    param.links.push(link);
                }
                iks.push(param);
        ...
        if (data.metadata.format === 'pmx') {
            ...
            function traverse(entry) {
                ...
                entry.visited = true;
                for (let i = 0, il = entry.children.length; i < il; i += 1) {
        ...
        function updateAttributes(attribute, morph, ratio) {
            for (let i = 0; i < morph.elementCount; i += 1) {
        ...
        for (let i = 0; i < data.metadata.constraintCount; i += 1) {
        ...
        geometry.setAttribute('skinIndex', new three_1.Uint16BufferAttribute(skinIndices, 4));
        geometry.setAttribute('skinWeight', new three_1.Float32BufferAttribute(skinWeights, 4));
        geometry.setIndex(indices);
        for (let i = 0, il = groups.length; i < il; i += 1) {
...
class MaterialBuilder {
    ...
    build(data, geometry /*, onProgress, onError */) {
        ...
        for (let i = 0; i < data.metadata.materialCount; i += 1) {
             ...
             */
            params.diffuse = new three_1.Color().fromArray(material.diffuse);
            params.opacity = material.diffuse[3];
            params.specular = new three_1.Color().fromArray(material.specular);
            ...
            if (data.metadata.format === 'pmd') {
                ...
                if (material.fileName) {
                    ...
                    if (fileNames.length > 1) {
                        ...
                        params.combine = extension === '.sph' ? three_1.MultiplyOperation : three_1.AddOperation;
                    }
                }
                // gradientMap
                ...
                if (material.envTextureIndex !== -1 && (material.envFlag === 1 || material.envFlag == 2)) {
                    ...
                    params.matcapCombine = material.envFlag === 1 ? three_1.MultiplyOperation : three_1.AddOperation;
                }
                // gradientMap
        ...
        if (data.metadata.format === 'pmx') {
            ...
            for (let i = 0, il = data.morphs.length; i < il; i += 1) {
                const morph = data.morphs[i];
    ...
    _getTGALoader() {
        if (this.tgaLoader === null) {
            if (TGALoader_1.TGALoader === undefined) {
                throw new Error('THREE.MMDLoader: Import TGALoader');
            }
            this.tgaLoader = new TGALoader_1.TGALoader(this.manager);
        }
...
class AnimationBuilder {
    ...
    build(vmd, mesh) {
        ...
        for (let i = 0, il = tracks2.length; i < il; i += 1) {
    ...
    buildSkeletalAnimation(vmd, mesh) {
        ...
        for (let i = 0, il = bones.length; i < il; i += 1) {
            boneNameDictionary[bones[i].name] = true;
        }
        for (let i = 0; i < vmd.metadata.motionCount; i += 1) {
        ...
        for (const key in motions) {
            ...
            tracks.push(this._createTrack(targetName + '.position', three_1.VectorKeyframeTrack, times, positions, pInterpolations));
            tracks.push(this._createTrack(targetName + '.quaternion', three_1.QuaternionKeyframeTrack, times, rotations, rInterpolations));
        }
        return new three_1.AnimationClip('', -1, tracks);
    }
    /**
    ...
    buildMorphAnimation(vmd, mesh) {
        ...
        for (let i = 0; i < vmd.metadata.morphCount; i += 1) {
    ...
    buildCameraAnimation(vmd) {
        ...
        tracks.push(this._createTrack('target.position', three_1.VectorKeyframeTrack, times, centers, cInterpolations));
        tracks.push(this._createTrack('.quaternion', three_1.QuaternionKeyframeTrack, times, quaternions, qInterpolations));
        tracks.push(this._createTrack('.position', three_1.VectorKeyframeTrack, times, positions, pInterpolations));
        tracks.push(this._createTrack('.fov', three_1.NumberKeyframeTrack, times, fovs, fInterpolations));
        return new three_1.AnimationClip('', -1, tracks);
    }
    // private method
        ...
        if (times.length > 2) {
            ...
            for (let aheadIndex = 2, endIndex = times.length; aheadIndex < endIndex; aheadIndex++) {
                ...
                if (aheadIndex > index) {
                    ...
                    for (let i = 0; i < interpolateStride; i += 1) {
...
// interpolation
class CubicBezierInterpolation extends three_1.Interpolant {
    ...
    interpolate_(i1, t0, t, t1) {
        ...
        if (stride === 4) {
            ...
            three_1.Quaternion.slerpFlat(result, 0, values, offset0, values, offset1, ratio);
        }
        else if (stride === 3) {
            // Vector3
...
class MMDToonMaterial extends three_1.ShaderMaterial {
    constructor(parameters) {
        ...
        this.lights = true;
        this.vertexShader = MMDToonShader_1.MMDToonShader.vertexShader;
        this.fragmentShader = MMDToonShader_1.MMDToonShader.fragmentShader;
        this.defines = Object.assign({}, MMDToonShader_1.MMDToonShader.defines);
