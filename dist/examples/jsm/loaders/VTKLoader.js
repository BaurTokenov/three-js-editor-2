...
class VTKLoader extends three_1.Loader {
    constructor(manager) {
        super(manager);
    }
    ...
    parse(data) {
        function parseASCII(data) {
            ...
            if (colors.length !== indices.length) {
                ...
                if (colors.length === numTriangles * 3) {
                    ...
                    geometry.setAttribute('color', new three_1.Float32BufferAttribute(newColors, 3));
                }
            }
            return geometry;
        }
        function parseBinary(data) {
            ...
            while (true) {
                ...
                if (line.indexOf('DATASET') === 0) {
                    ...
                    let indicesIndex = 0;
                    let pointIndex = state.next;
                    for (let i = 0; i < numberOfStrips; i += 1) {
            ...
            geometry.setIndex(new three_1.BufferAttribute(indices, 1));
            geometry.setAttribute('position', new three_1.BufferAttribute(points, 3));
            if (normals.length === points.length) {
                geometry.setAttribute('normal', new three_1.BufferAttribute(normals, 3));
            }
            return geometry;
        }
        ...
        function parseXML(stringFile) {
            ...
            function parseDataArray(ele, compressed) {
                ...
                if (ele.attributes.format === 'binary' && compressed) {
                    ...
                    for (let i = 0; i < dataOffsets.length - 1; i += 1) {
                            ...
                            case 'Strips':
                                {
                                    ...
                                    if (numberOfStrips > 0) {
                                        ...
                                        indices = new Uint32Array(3 * size - 9 * numberOfStrips);
                                        let indicesIndex = 0;
                                        for (let i = 0, len = numberOfStrips; i < len; i += 1) {
                                            const strip = [];
        ...
        const meta = three_1.LoaderUtils.decodeText(new Uint8Array(data, 0, 250)).split('\n');
        if (meta[0].indexOf('xml') !== -1) {
            return parseXML(three_1.LoaderUtils.decodeText(data));
        }
        else if (meta[2].includes('ASCII')) {
            return parseASCII(three_1.LoaderUtils.decodeText(data));
        }
        else {
