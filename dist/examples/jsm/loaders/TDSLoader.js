 ...
 * @constructor
 */
class TDSLoader extends three_1.Loader {
    constructor(manager) {
        super(manager);
        this.debug = false;
        this.group = null;
    ...
    readMaterialEntry(chunk, path) {
        let next = chunk.readChunk();
        const material = new three_1.MeshPhongMaterial();
        while (next) {
            if (next.id === MAT_NAME) {
                ...
                material.blending = three_1.AdditiveBlending;
            }
    ...
    readFaceArray(chunk, mesh) {
        ...
        if (mesh.material.length === 1)
            mesh.material = mesh.material[0]; // for backwards compatibility
    }
    /**
    ...
    readMap(chunk, path) {
        let next = chunk.readChunk();
        let texture = {};
        const loader = new three_1.TextureLoader(this.manager);
