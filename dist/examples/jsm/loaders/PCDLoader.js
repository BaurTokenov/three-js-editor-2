...
class PCDLoader extends three_1.Loader {
    constructor(manager) {
        super(manager);
    ...
    parse(data, url) {
        ...
        const textData = three_1.LoaderUtils.decodeText(new Uint8Array(data));
        ...
        if (position.length > 0)
            geometry.setAttribute('position', new three_1.Float32BufferAttribute(position, 3));
        if (normal.length > 0)
            geometry.setAttribute('normal', new three_1.Float32BufferAttribute(normal, 3));
        if (color.length > 0)
            geometry.setAttribute('color', new three_1.Float32BufferAttribute(color, 3));
        geometry.computeBoundingSphere();
        // build material
        const material = new three_1.PointsMaterial({ size: 0.005 });
        if (color.length > 0) {
        ...
        // build point cloud
        const mesh = new three_1.Points(geometry, material);
