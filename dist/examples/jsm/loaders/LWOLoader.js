...
const IFFParser_1 = require("./lwo/IFFParser");
let _lwoTree;
class LWOLoader extends three_1.Loader {
    ...
    parse(iffBuffer, path, modelName) {
        _lwoTree = new IFFParser_1.IFFParser().parse(iffBuffer);
...
class LWOTreeParser {
    ...
    parseMesh(geometry, layer) {
        ...
        if (layer.geometry.type === 'points')
            mesh = new three_1.Points(geometry, materials);
        else if (layer.geometry.type === 'lines')
            mesh = new three_1.LineSegments(geometry, materials);
        else
            mesh = new three_1.Mesh(geometry, materials);
        if (layer.name)
        ...
        if (type === 'points' || type === 'lines') {
            materials.forEach(function (mat, i) {
                ...
                if (type === 'points') {
                    spec.size = 0.1;
                    spec.map = mat.map;
                    materials[i] = new three_1.PointsMaterial(spec);
                }
                else if (type === 'lines') {
                    materials[i] = new three_1.LineBasicMaterial(spec);
                }
            });
        }
    ...
    duplicateUVs(geometry, materials) {
        ...
        if (!duplicateUVs)
            return;
        geometry.setAttribute('uv2', new three_1.BufferAttribute(geometry.attributes.uv.array, 2));
    }
}
class MaterialParser {
    ...
    parseMaterialLwo2(materialData, name /*, textures*/) {
        ...
        params = Object.assign(params, attributes);
        return new three_1.MeshPhongMaterial(params);
    }
    ...
    getSide(attributes) {
        if (!attributes.side)
            return three_1.BackSide;
        switch (attributes.side) {
            case 0:
            case 1:
                return three_1.BackSide;
            case 2:
                return three_1.FrontSide;
            case 3:
                return three_1.DoubleSide;
        }
    }
            ...
            switch (name) {
                ...
                case 'Normal':
                    ...
                    if (node.amplitude !== undefined)
                        maps.normalScale = new three_1.Vector2(node.amplitude, node.amplitude);
                    break;
                case 'Bump':
    ...
    parseAttributes(attributes, maps) {
        ...
        if (attributes.Color && !maps.map) {
            params.color = new three_1.Color().fromArray(attributes.Color.value);
        }
        else
            params.color = new three_1.Color();
    ...
    parseStandardAttributes(params, attributes, maps) {
        if (attributes.Luminous) {
            ...
            if (attributes['Luminous Color'] && !maps.emissive) {
                params.emissive = new three_1.Color().fromArray(attributes['Luminous Color'].value);
            }
            else {
                params.emissive = new three_1.Color(0x808080);
            }
        }
    ...
    parsePhongAttributes(params, attributes, maps) {
        ...
        if (attributes.Reflection) {
            ...
            params.combine = three_1.AddOperation;
        }
        ...
        if (!attributes.Roughness && attributes.Specular && !maps.specularMap) {
            if (attributes['Color Highlight']) {
                params.specular = new three_1.Color()
                ...
                params.specular = new three_1.Color().setScalar(attributes.Specular.value);
            }
        }
    ...
    parseEnvMap(connections, maps, attributes) {
        if (connections.envMap) {
            ...
            if (attributes.transparent && attributes.opacity < 0.999) {
                envMap.mapping = three_1.EquirectangularRefractionMapping;
                ...
                envMap.mapping = three_1.EquirectangularReflectionMapping;
            maps.envMap = envMap;
        }
    }
    ...
    getWrappingType(num) {
        switch (num) {
            case 0:
                ...
                return three_1.ClampToEdgeWrapping;
            case 1:
                return three_1.RepeatWrapping;
            case 2:
                return three_1.MirroredRepeatWrapping;
            case 3:
                return three_1.ClampToEdgeWrapping;
        }
    }
    getMaterialType(nodeData) {
        ...
        return three_1.MeshPhongMaterial;
    }
}
class GeometryParser {
    parse(geoData, layer) {
        const geometry = new three_1.BufferGeometry();
        geometry.setAttribute('position', new three_1.Float32BufferAttribute(geoData.points, 3));
    ...
    parseUVs(geometry, layer) {
        ...
        geometry.setAttribute('uv', new three_1.Float32BufferAttribute(remappedUVs, 2));
    }
    parseMorphTargets(geometry, layer) {
        ...
        for (const name in layer.morphTargets) {
            ...
            geometry.morphAttributes.position[num] = new three_1.Float32BufferAttribute(remappedPoints, 3);
