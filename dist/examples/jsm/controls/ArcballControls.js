...
exports.ArcballControls = void 0;
...
const _transformation = {
    camera: new three_1.Matrix4(),
    gizmos: new three_1.Matrix4()
};
//events
 ...
 */
class ArcballControls extends three_1.EventDispatcher {
    constructor(camera, domElement, scene = null) {
                    ...
                    switch (mouseOp) {
                        ...
                        case 'FOV':
                            if (this.camera.isPerspectiveCamera) {
                                ...
                                const newDistance = y / Math.tan(three_1.MathUtils.DEG2RAD * (newFov / 2));
                                size = x / newDistance;
                                this.setFov(newFov);
                    ...
                    case STATE.FOV:
                        if (this.enableZoom && this.camera.isPerspectiveCamera) {
                            if (restart) {
                                ...
                                //check min and max distance
                                xNew = three_1.MathUtils.clamp(xNew, this.minDistance, this.maxDistance);
                                const y = x * Math.tan(three_1.MathUtils.DEG2RAD * this._fovState * 0.5);
                                //calculate new fov
                                let newFov = three_1.MathUtils.RAD2DEG * (Math.atan(y / xNew) * 2);
                                //check min and max fov
                                newFov = three_1.MathUtils.clamp(newFov, this.minFov, this.maxFov);
                                const newDistance = y / Math.tan(three_1.MathUtils.DEG2RAD * (newFov / 2));
                                size = x / newDistance;
        ...
        this.onRotateMove = () => {
            if (this.enabled && this.enableRotate) {
                ...
                if (!this.enablePan) {
                    rotationPoint = new three_1.Vector3().setFromMatrixPosition(this._gizmoMatrixState);
                }
                else {
                    this._v3_2.setFromMatrixPosition(this._gizmoMatrixState);
                ...
                const amount = three_1.MathUtils.DEG2RAD * (this._startFingerRotation - this._currentFingerRotation);
        ...
        this.calculateTbRadius = (camera) => {
            ...
            if (camera.type == 'PerspectiveCamera') {
                const halfFovV = three_1.MathUtils.DEG2RAD * camera.fov * 0.5; //vertical fov/2 in radians
        ...
        this.drawGrid = () => {
            if (this.scene != null) {
                ...
                if (this.camera.isOrthographicCamera) {
                    ...
                    const halfFovV = three_1.MathUtils.DEG2RAD * this.camera.fov * 0.5;
        ...
        this.makeGizmos = (tbCenter, tbRadius) => {
            const curve = new three_1.EllipseCurve(0, 0, tbRadius, tbRadius);
            const points = curve.getPoints(this._curvePts);
            //geometry
            const curveGeometry = new three_1.BufferGeometry().setFromPoints(points);
            //material
            const curveMaterialX = new three_1.LineBasicMaterial({ color: 0xff8080, fog: false, transparent: true, opacity: 0.6 });
            const curveMaterialY = new three_1.LineBasicMaterial({ color: 0x80ff80, fog: false, transparent: true, opacity: 0.6 });
            const curveMaterialZ = new three_1.LineBasicMaterial({ color: 0x8080ff, fog: false, transparent: true, opacity: 0.6 });
            //line
            const gizmoX = new three_1.Line(curveGeometry, curveMaterialX);
            const gizmoY = new three_1.Line(curveGeometry, curveMaterialY);
            const gizmoZ = new three_1.Line(curveGeometry, curveMaterialZ);
        ...
        this.setFov = (value) => {
            if (this.camera.isPerspectiveCamera) {
                this.camera.fov = three_1.MathUtils.clamp(value, this.minFov, this.maxFov);
                this.camera.updateProjectionMatrix();
            }
        };
        /**
        ...
        this.unprojectOnTbSurface = (camera, cursorX, cursorY, canvas, tbRadius) => {
            if (camera.type == 'OrthographicCamera') {
                ...
                if (delta >= 0) {
                    ...
                    const angle = three_1.MathUtils.RAD2DEG * this._v2_1.angle();
                    if (angle >= 45) {
        ...
        this.update = () => {
            ...
            if (this.camera.isOrthographicCamera) {
                ...
                if (this.camera.zoom > this.maxZoom || this.camera.zoom < this.minZoom) {
                    const newZoom = three_1.MathUtils.clamp(this.camera.zoom, this.minZoom, this.maxZoom);
                ...
                if (distance > this.maxDistance + EPS || distance < this.minDistance - EPS) {
                    const newDistance = three_1.MathUtils.clamp(distance, this.minDistance, this.maxDistance);
                    this.applyTransformMatrix(this.scale(newDistance / distance, this._gizmos.position));
                    this.updateMatrixState();
                }
                //check fov
                if (this.camera.fov < this.minFov || this.camera.fov > this.maxFov) {
                    this.camera.fov = three_1.MathUtils.clamp(this.camera.fov, this.minFov, this.maxFov);
                    this.camera.updateProjectionMatrix();
                }
        ...
        this.setStateFromJSON = (json) => {
            ...
            if (state.arcballState != undefined) {
                ...
                this._gizmos.updateMatrix();
                this._tbRadius = this.calculateTbRadius(this.camera);
                const gizmoTmp = new three_1.Matrix4().copy(this._gizmoMatrixState0);
                this.makeGizmos(this._gizmos.position, this._tbRadius);
        ...
        this._currentTarget = new three_1.Vector3();
        this.radiusFactor = 0.67;
        this.mouseActions = [];
        this._mouseOp = null;
        //global vectors and matrices that are used in some operations to avoid creating new objects every time (e.g. every time cursor moves)
        this._v2_1 = new three_1.Vector2();
        this._v3_1 = new three_1.Vector3();
        this._v3_2 = new three_1.Vector3();
        this._m4_1 = new three_1.Matrix4();
        this._m4_2 = new three_1.Matrix4();
        this._quat = new three_1.Quaternion();
        //transformation matrices
        this._translationMatrix = new three_1.Matrix4(); //matrix for translation operation
        this._rotationMatrix = new three_1.Matrix4(); //matrix for rotation operation
        this._scaleMatrix = new three_1.Matrix4(); //matrix for scaling operation
        this._rotationAxis = new three_1.Vector3(); //axis for rotate operation
        //camera state
        this._cameraMatrixState = new three_1.Matrix4();
        this._cameraProjectionState = new three_1.Matrix4();
        this._fovState = 1;
        this._upState = new three_1.Vector3();
        this._zoomState = 1;
        this._nearPos = 0;
        this._farPos = 0;
        this._gizmoMatrixState = new three_1.Matrix4();
        //initial values
        this._up0 = new three_1.Vector3();
        this._zoom0 = 1;
        this._fov0 = 0;
        this._initialNear = 0;
        this._nearPos0 = 0;
        this._initialFar = 0;
        this._farPos0 = 0;
        this._cameraMatrixState0 = new three_1.Matrix4();
        this._gizmoMatrixState0 = new three_1.Matrix4();
        //pointers array
        this._button = -1;
        this._touchStart = [];
        this._touchCurrent = [];
        ...
        //cursor positions
        this._currentCursorPosition = new three_1.Vector3();
        ...
        this._cursorPosPrev = new three_1.Vector3(); //cursor position when previous rotate operation has been detected
        this._cursorPosCurr = new three_1.Vector3(); //cursor position when current rotate operation has been detected
    ...
    setTbRadius(value) {
        ...
        this._tbRadius = this.calculateTbRadius(this.camera);
        const curve = new three_1.EllipseCurve(0, 0, this._tbRadius, this._tbRadius);
        const points = curve.getPoints(this._curvePts);
        const curveGeometry = new three_1.BufferGeometry().setFromPoints(points);
        for (const gizmo in this._gizmos.children) {
            this._gizmos.children[gizmo].geometry = curveGeometry;
