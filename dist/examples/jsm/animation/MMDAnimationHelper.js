...
const CCDIKSolver_1 = require("../animation/CCDIKSolver");
const MMDPhysics_1 = require("../animation/MMDPhysics");
/**
...
class MMDAnimationHelper {
    ...
    constructor(params = {}) {
        this.meshes = [];
        this.camera = null;
        this.cameraTarget = new three_1.Object3D();
    ...
    pose(mesh, vpd, params = {}) {
        ...
        for (let i = 0, il = boneParams.length; i < il; i += 1) {
    ...
    enable(key, enabled) {
        ...
        if (key === 'physics') {
            for (let i = 0, il = this.meshes.length; i < il; i += 1) {
    ...
    _setupMeshAnimation(mesh, animation) {
        ...
        if (animation !== undefined) {
            ...
            objects.mixer = new three_1.AnimationMixer(mesh);
    ...
    _setupCameraAnimation(camera, animation) {
        ...
        objects.mixer = new three_1.AnimationMixer(camera);
        for (let i = 0, il = animations.length; i < il; i += 1) {
            objects.mixer.clipAction(animations[i]).play();
        }
    }
    ...
    _animatePMXMesh(mesh, sortedBonesData, ikSolver, grantSolver) {
        ...
        for (let i = 0, il = sortedBonesData.length; i < il; i += 1) {
    ...
    _optimizeIK(mesh, physicsEnabled) {
        ...
        for (let i = 0, il = iks.length; i < il; i += 1) {
            const ik = iks[i];
    ...
    _createCCDIKSolver(mesh) {
        if (CCDIKSolver_1.CCDIKSolver === undefined) {
            throw new Error('THREE.MMDAnimationHelper: Import CCDIKSolver.');
        }
        return new CCDIKSolver_1.CCDIKSolver(mesh, mesh.geometry.userData.MMD.iks);
    }
    _createMMDPhysics(mesh, params) {
        if (MMDPhysics_1.MMDPhysics === undefined) {
            throw new Error('THREE.MMDPhysics: Import MMDPhysics.');
        }
        return new MMDPhysics_1.MMDPhysics(mesh, mesh.geometry.userData.MMD.rigidBodies, mesh.geometry.userData.MMD.constraints, params);
    }
    /*
    ...
    _syncDuration() {
        ...
        for (let i = 0, il = meshes.length; i < il; i += 1) {
        ...
        if (camera !== null) {
            const mixer = this.objects.get(camera).mixer;
            if (mixer !== undefined) {
                for (let i = 0, il = mixer._actions.length; i < il; i += 1) {
    ...
    _updatePropertyMixersBuffer(mesh) {
        ...
        for (let i = 0, il = propertyMixers.length; i < il; i += 1) {
...
function getQuaternion() {
    if (_quaternionIndex >= _quaternions.length) {
        _quaternions.push(new three_1.Quaternion());
    }
...
class GrantSolver {
    ...
    update() {
        ...
        for (let i = 0, il = grants.length; i < il; i += 1) {
