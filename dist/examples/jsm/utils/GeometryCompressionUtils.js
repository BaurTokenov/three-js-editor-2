...
exports.compressUvs = exports.compressPositions = exports.compressNormals = void 0;
const three_1 = require("three");
const PackedPhongMaterial_1 = require("./PackedPhongMaterial");
/**
...
function compressNormals(mesh, encodeMethod) {
    ...
    if (encodeMethod == 'DEFAULT') {
        ...
        mesh.geometry.setAttribute('normal', new three_1.BufferAttribute(result, 3, true));
        ...
        mesh.geometry.setAttribute('normal', new three_1.BufferAttribute(result, 2, true));
        mesh.geometry.attributes.normal.bytes = result.length * 1;
    }
    ...
    // modify material
    if (!(mesh.material instanceof PackedPhongMaterial_1.PackedPhongMaterial)) {
        mesh.material = new PackedPhongMaterial_1.PackedPhongMaterial().copy(mesh.material);
    }
...
function compressPositions(mesh) {
    ...
    mesh.geometry.setAttribute('position', new three_1.BufferAttribute(quantized, 3));
...
function compressUvs(mesh) {
    ...
    if (range.min >= -1.0 && range.max <= 1.0) {
        ...
        mesh.geometry.setAttribute('uv', new three_1.BufferAttribute(result, 2, true));
        ...
        mesh.geometry.setAttribute('uv', new three_1.BufferAttribute(result.quantized, 2));
        mesh.geometry.attributes.uv.isPacked = true;
        mesh.geometry.attributes.uv.needsUpdate = true;
...
function quantizedEncode(array, bytes) {
    ...
    const decodeMat = new three_1.Matrix4();
    ...
    decodeMat.scale(new three_1.Vector3((max[0] - min[0]) / segments, (max[1] - min[1]) / segments, (max[2] - min[2]) / segments));
...
function quantizedEncodeUV(array, bytes) {
    ...
    const decodeMat = new three_1.Matrix3();
