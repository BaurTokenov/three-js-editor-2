 ...
 */
const _cb = new three_1.Vector3(), _ab = new three_1.Vector3();
class SimplifyModifier {
    modify(geometry, count) {
        ...
        for (let i = 0; i < positionAttribute.count; i += 1) {
            const v = new three_1.Vector3().fromBufferAttribute(positionAttribute, i);
        ...
        //
        const simplifiedGeometry = new three_1.BufferGeometry();
        const position = [];
        index = [];
        //
...
function computeEdgeCollapseCost(u, v) {
    ...
    for (let i = 0, il = u.faces.length; i < il; i += 1) {
...
function computeEdgeCostAtVertex(v) {
    ...
    for (let i = 0; i < v.neighbors.length; i += 1) {
...
function collapse(vertices, faces, u, v) {
    ...
    const tmpVertices = [];
    for (let i = 0; i < u.neighbors.length; i += 1) {
        tmpVertices.push(u.neighbors[i]);
    }
    // delete triangles on edge uv:
    for (let i = u.faces.length - 1; i >= 0; i -= 1) {
    ...
    for (let i = 0; i < tmpVertices.length; i += 1) {
