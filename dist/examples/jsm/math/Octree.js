...
const Capsule_1 = require("../math/Capsule");
const _v1 = new three_1.Vector3();
const _v2 = new three_1.Vector3();
const _plane = new three_1.Plane();
const _line1 = new three_1.Line3();
const _line2 = new three_1.Line3();
const _sphere = new three_1.Sphere();
const _capsule = new Capsule_1.Capsule();
class Octree {
    constructor(box) {
        this.triangles = [];
        this.box = box;
    ...
    capsuleIntersect(capsule) {
        ...
        if (hit) {
            const collisionVector = _capsule.getCenter(new three_1.Vector3()).sub(capsule.getCenter(_v1));
            const depth = collisionVector.length();
            return { normal: collisionVector.normalize(), depth: depth };
        }
        return false;
        ...
        group.traverse((obj) => {
            if (obj.isMesh === true) {
                ...
                for (let i = 0; i < positionAttribute.count; i += 3) {
                    const v1 = new three_1.Vector3().fromBufferAttribute(positionAttribute, i);
                    const v2 = new three_1.Vector3().fromBufferAttribute(positionAttribute, i + 1);
                    const v3 = new three_1.Vector3().fromBufferAttribute(positionAttribute, i + 2);
                    v1.applyMatrix4(obj.matrixWorld);
                    v2.applyMatrix4(obj.matrixWorld);
                    v3.applyMatrix4(obj.matrixWorld);
                    this.addTriangle(new three_1.Triangle(v1, v2, v3));
                }
                if (isTemp) {
                    geometry.dispose();
                }
