...
class LightProbeGenerator {
    ...
    static fromCubeTexture(cubeTexture) {
        let totalWeight = 0;
        const coord = new three_1.Vector3();
        const dir = new three_1.Vector3();
        const color = new three_1.Color();
        const shBasis = [0, 0, 0, 0, 0, 0, 0, 0, 0];
        const sh = new three_1.SphericalHarmonics3();
        ...
        for (let faceIndex = 0; faceIndex < 6; faceIndex++) {
            ...
            for (let i = 0, il = data.length; i < il; i += 4) { // RGBA assumed
                ...
                three_1.SphericalHarmonics3.getBasisAt(dir, shBasis);
                // accummuulate
                for (let j = 0; j < 9; j++) {
        ...
        return new three_1.LightProbe(sh);
    }
...
function convertColorToLinear(color, encoding) {
    switch (encoding) {
        case three_1.sRGBEncoding:
            color.convertSRGBToLinear();
            break;
        case three_1.LinearEncoding:
            break;
        default:
