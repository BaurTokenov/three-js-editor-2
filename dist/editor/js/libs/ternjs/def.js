...
(function (mod) {
                ...
                (function (i) {
                    ...
                    if (arg instanceof infer.Fn && arg.args && arg.args.length)
                        addEffect(type, function (_self, fArgs) {
                            var fArg = fArgs[i];
                            if (fArg)
                                fArg.propagate(new infer.IsCallee(infer.cx().topScope, arg.args, null, infer.ANull));
                        });
                })(i);
        return type;
    }
    ...
    var parseEffect = (exports.parseEffect = function (effect, fn) {
        ...
        if (effect.indexOf('propagate ') == 0) {
            ...
            if (customFunc)
                addEffect(fn, m[2] ? customFunc(m[2]) : customFunc);
        }
    ...
    function isSimpleAnnotation(spec) {
        if (!spec['!type'] || /^(fn\(|\[)/.test(spec['!type']))
            return false;
    ...
    function passOne(base, spec, path) {
        ...
        for (var name in spec)
            if (hop(spec, name) && name.charCodeAt(0) != 33) {
                var inner = spec[name];
                if (typeof inner == 'string' || isSimpleAnnotation(inner))
                    continue;
