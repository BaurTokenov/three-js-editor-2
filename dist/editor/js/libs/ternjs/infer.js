...
(function (root, mod) {
    ...
    var toString = (exports.toString = function (type, maxDepth, parent) {
        if (!type || type == parent || (maxDepth && maxDepth < -3))
            return '?';
    ...
    AVal.prototype = extend(ANull, {
        addType: function (type, weight) {
            ...
            if (this.maxWeight < weight) {
                ...
                if (this.types.length == 1 && this.types[0] == type)
                    return;
                this.types.length = 0;
            }
        ...
        propagate: function (target, weight) {
            if (target == ANull || (target instanceof Type && this.forward && this.forward.length > 2))
                return;
            if (weight && weight != WG_DEFAULT)
    ...
    function similarType(a, b, depth) {
        ...
        if (a.constructor == Arr) {
            ...
            if (!innerB || similarType(innerA, innerB, depth + 1))
                return b;
        }
    ...
    function canonicalType(types) {
        ...
        for (var i = 0; i < types.length; ++i) {
            var tp = types[i];
            if (tp instanceof Arr)
                ++arrays;
            else if (tp instanceof Fn)
                ++fns;
        ...
        for (var i = 0; i < types.length; ++i) {
            ...
            if (arrays) {
                ...
                for (var j = 0; j < tp.args.length; ++j)
                    if (!tp.args[j].isEmpty())
                        ++score;
                if (!tp.retval.isEmpty())
                    ++score;
            }
            else if (objs) {
                score = tp.name ? 100 : 2;
    ...
    var IsCtor = (exports.IsCtor = constraint('target, noReuse', {
        addType: function (f, weight) {
            if (!(f instanceof Fn))
                return;
            if (cx.parent && !cx.parent.options.reuseInstances)
                this.noReuse = true;
    ...
    var IsProto = (exports.IsProto = constraint('ctor, target', {
        addType: function (o, _weight) {
            if (!(o instanceof Obj))
                return;
            if ((this.count = (this.count || 0) + 1) > 8)
                return;
    ...
    Obj.prototype = extend(Type.prototype, {
        ...
        defProp: function (prop, originNode) {
            ...
            if (found) {
                if (originNode && !found.originNode)
        ...
        maybeUnregProtoPropHandler: function () {
            ...
            if (!this.proto || (this.onNewProp && this.onNewProp.length))
                return;
        ...
        defVar: function (name, originNode) {
            for (var s = this;; s = s.proto) {
                ...
                if (found)
                    return found;
                if (!s.prev)
                    return s.defProp(name, originNode);
            }
        },
    });
    ...
    var inferExprVisitor = {
        ...
        BinaryExpression: ret(function (node, scope, c) {
            if (node.operator == '+') {
                ...
                if (lhs.hasType(cx.str) || rhs.hasType(cx.str))
                    return cx.str;
                if (lhs.hasType(cx.num) && rhs.hasType(cx.num))
                    return cx.num;
                var result = new AVal();
        ...
        CallExpression: fill(function (node, scope, c, out) {
            ...
            if (node.callee.type == 'MemberExpression') {
                ...
                if ((pName == 'call' || pName == 'apply') && scope.fnType && scope.fnType.args.indexOf(self) > -1)
        ...
        Identifier: ret(function (node, scope) {
            if (node.name == 'arguments' && scope.fnType && !(node.name in scope.props))
                scope.defProp(node.name, scope.fnType.originNode).addType(new Arr((scope.fnType.arguments = new AVal())));
    ...
    var inferWrapper = walk.make({
        ...
        ReturnStatement: function (node, scope, c) {
            ...
            var output = ANull;
            if (scope.fnType) {
                if (scope.fnType.retval == ANull)
                    scope.fnType.retval = new AVal();
    ...
    Fn.prototype.purge = function (test) {
        if (Obj.prototype.purge.call(this, test))
            return;
    ...
    var typeFinder = {
        ArrayExpression: function (node, scope) {
            ...
            for (var i = 0; i < node.elements.length; ++i) {
                var elt = node.elements[i];
                if (elt)
                    findType(elt, scope).propagate(eltval);
            }
            return new Arr(eltval);
        },
    ...
    var findTypeFromContext = {
        ...
        CallExpression: function (parent, node, get) {
            for (var i = 0; i < parent.arguments.length; i += 1) {
        ...
        ReturnStatement: function (_parent, node, get) {
            ...
            if (fnNode) {
                ...
                if (fnType)
                    return fnType.retval.getType();
            }
        },
        VariableDeclaration: function (parent, node, get) {
            for (var i = 0; i < parent.declarations.length; i += 1) {
    ...
    exports.findPropRefs = function (ast, scope, objType, propName, f) {
        walk.simple(ast, {
            MemberExpression: function (node, scope) {
                if (node.computed || node.property.name != propName)
                    return;
                if (findType(node.object, scope).getType() == objType)
                    f(node.property);
            },
            ObjectExpression: function (node, scope) {
                if (findType(node, scope).getType() != objType)
                    return;
                for (var i = 0; i < node.properties.length; ++i)
