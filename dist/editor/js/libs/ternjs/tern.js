...
(function (root, mod) {
    ...
    Server.prototype = signal.mixin({
        addFile: function (name, /*optional*/ text, parent) {
            ...
            if (parent && !(parent in this.fileMap))
                parent = null;
    ...
    function doRequest(srv, doc, c) {
        ...
        analyzeAll(srv, timeBudget, function (err) {
            ...
            infer.withContext(srv.cx, timeBudget
                ? function () {
    ...
    function analyzeAll(srv, timeBudget, c) {
        if (srv.pending)
            return waitOnFetch(srv, timeBudget, c);
        ...
        for (var i = 0; i < srv.files.length;) {
            ...
            for (; i < srv.files.length; ++i) {
                ...
                else if (file.scope == null && !file.excluded)
                    toAnalyze.push(file);
            }
        ...
        infer.withContext(srv.cx, function () {
            ...
            tieTogether: if (inObject || atFunction) {
                ...
                if (inObject) {
                    ...
                    if (inner == scopeStart || !inner.fnType)
                        break tieTogether;
    ...
    function pointInProp(objNode, point) {
        for (var i = 0; i < objNode.properties.length; i += 1) {
    ...
    function findCompletions(srv, query, file) {
        ...
        if (srv.passes.completion)
            for (var i = 0; i < srv.passes.completion.length; i += 1) {
        ...
        function gather(prop, obj, depth, addInfo) {
            ...
            if ((objLit || query.omitObjectPrototype !== false) && obj == srv.cx.protos.Object && !word)
                return;
            if (query.filter !== false && word && (query.caseInsensitive ? prop.toLowerCase() : prop).indexOf(word) !== 0)
                return;
            if (ignoreObj && ignoreObj.props[prop])
                return;
    ...
    var findExpr = (exports.findQueryExpr = function (file, query, wide) {
        ...
        if (query.variable) {
            ...
            return { node: { type: 'Identifier', name: query.variable, start: query.end, end: query.end + 1 }, state: scope };
        }
        else {
    ...
    function findTypeAt(srv, query, file) {
        ...
        if (type)
            storeTypeDocs(type, result);
        if (!result.doc && exprType.doc)
            result.doc = exprType.doc;
        return clean(result);
    }
        ...
        if (scope.originNode) {
            ...
            if (checkShadowing) {
                for (var prev = scope.prev; prev; prev = prev.prev)
                    if (checkShadowing in prev.props)
                        break;
                if (prev)
